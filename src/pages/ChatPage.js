...
const initialChats = []; // Убраны статические данные
const initialGroups = []; // Убраны статические данные
        ...
        chat.id === activeChat.id
          ? {
              ...
              messages: [
                ...
                {
                  ...messageData,
                  received: false, // Сообщение отправлено текущим пользователем
                },
              ],
          ...
          chat.id === message.receiverId || chat.id === message.senderId
            ? {
                ...
                messages: [
                  ...
                  {
                    ...message,
                    received: message.senderId !== currentUser.id, // Правильно определяем, чужое ли сообщение
                  },
                ],
              }
            : chat
  ...
  return (
    <div className="fixed inset-0 bg-black">
      <div className="flex h-full">
        ...
        <div className="w-20 border-r border-white/10 flex flex-col items-center py-4 space-y-4">
          <button
            onClick={() => setView("chats")}
            className={`p-3 rounded-xl transition-colors ${view === "chats" ? "bg-white/20" : "hover:bg-white/10"}`}
          >
            <Users size={24} className="text-white" />
          </button>
          <div className="w-10 border-b border-white/10"></div>
          {groups.map((group) => (
            <button
              key={group.id}
              onClick={() => {
              ...
              className={`relative p-1 rounded-xl transition-colors ${activeGroup === group.id ? "bg-white/20" : "hover:bg-white/10"}`}
            >
              <img
                src={group.avatar}
                alt={group.name}
              ...
              {group.unread > 0 && (
                <div className="absolute -top-1 -right-1 w-5 h-5 bg-white/20 rounded-full flex items-center justify-center">
                  <span className="text-xs text-white">{group.unread}</span>
                </div>
              )}
            </button>
          ))}
        ...
        {activeConversation ? (
          <div className="flex-1 bg-black flex flex-col">
            ...
            {/* Контейнер сообщений с фиксированной высотой и прокруткой */}
            ...
            {/* Форма отправки сообщения (прижата к низу) */}
...
const ChatItem = ({ name, message, time, unread, active, online, onClick }) => (
  ...
  >
    ...
    <div className="flex-1 text-left min-w-0"> {/* Добавлен min-w-0 для корректного укорачивания */}
      <div className="flex items-center justify-between">
        <h3 className="text-white font-medium truncate">{name}</h3>
        <span className="text-xs text-gray-400 whitespace-nowrap">{time}</span>
      </div>
      <p className="text-sm text-gray-400 truncate">{message}</p> {/* Укорачивание текста */}
    </div>
    {unread > 0 && (
...
const Message = ({ content, time, received, avatar, isRead }) => {
  return (
    <div className={`flex ${received ? "justify-start" : "justify-end"} items-end gap-3 mb-4`}>
      {received && (
        <img
          src={avatar}
          alt="User Avatar"
      ...
      <div
        className={`max-w-[60ch] ${received ? "bg-gray-200" : "bg-blue-500"} 
        rounded-2xl px-4 py-2 space-y-1 break-words whitespace-pre-wrap`}
      >
